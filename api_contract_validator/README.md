...
| Task | Phase | Baseline (Qwen2.5-72B) | Trained (Qwen2.5-1.5B + LoRA) | Δ |
|---|---|---|---|---|
| `find_type_mismatches` | 1 | 0.75 | _(after training)_ | _Δ_ |
| `validate_nested_objects` | 1 | 0.99 | _(after training)_ | _Δ_ |
| `detect_breaking_changes` | 1 | **0.01** | _(after training)_ | _largest delta expected here_ |
| `validate_response_schema` | 1 | 0.99 | _(after training)_ | _Δ_ |
| `validate_cross_field_constraints` | 1 | 0.86 | _(after training)_ | _Δ_ |
| `validate_auth_request` | 1 | 0.99 | _(after training)_ | _Δ_ |
| `trace_downstream_blast_radius` | 2 | 0.67 | _(after training)_ | _Δ_ |
| `propose_backward_compat_fix` | 3 | 0.99 | _(after training)_ | _Δ_ |
| `multi_service_cascade_fix` | 2+3 | 0.99 | _(after training)_ | _Δ_ |
| **Mean** |   | **0.82** | _(after training)_ | _Δ_ |

Full per-step rewards in [`../baseline_scores.json`](../baseline_scores.json) (before) and [`../trained_scores.json`](../trained_scores.json) (after — generated post-training).
...
**Where to expect the biggest delta**: `detect_breaking_changes` at 0.01 — the 72B baseline finds the right field paths (proximity hits) but never predicts `violation_type='breaking_change'` correctly. GRPO should lift this substantially because the env's grader gives a clear +1.0 vs +0.3 vs -0.3 signal that the policy can directly optimise for.
